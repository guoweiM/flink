...
public class SavepointLoader {
	...
	public static CompletedCheckpoint loadAndValidateSavepoint(
		...
		if (savepoint.getVersion() == SavepointV2.VERSION) {
			// generate mapping from operator to task so that we can verify the operators properties against the actual task
			Map<JobVertexID, ExecutionJobVertex> operatorToTaskMap = new HashMap<>();
			for (Map.Entry<JobVertexID, ExecutionJobVertex> task : tasks.entrySet()) {
				for (JobVertexID operatorID : task.getValue().getOperatorIDs()) {
					operatorToTaskMap.put(operatorID, task.getValue());
				}
			}

			// (2) validate it (parallelism, etc)
			for (TaskState operatorState : savepoint.getTaskStates()) {
				ExecutionJobVertex executionJobVertex = operatorToTaskMap.get(operatorState.getJobVertexID());
				...
				if (executionJobVertex != null) {
					if (executionJobVertex.getMaxParallelism() == operatorState.getMaxParallelism()
						...
						taskStates.put(operatorState.getJobVertexID(), operatorState);
					}
				}else if (allowNonRestoredState) {
					LOG.info("Skipping savepoint state for operator {}.", operatorState.getJobVertexID());
				} else {
					String msg = String.format("Failed to rollback to savepoint %s. " +
							...
							"option on the CLI.",
						savepointPath, operatorState.getJobVertexID());
