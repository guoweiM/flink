...
class KafkaWriter<T> implements Writer<T, FlinkKafkaProducer.KafkaTransactionState> {
	...
	private static final ListStateDescriptor<Integer> MANAGED_SETS_NUM =
		new ListStateDescriptor<>("managed-sets-num", Integer.class);
	...
	private final ListState<Integer> managedSetNum;
	...
	public KafkaWriter(
		...
		this.managedSetNum = initialContext.getUnionListState(MANAGED_SETS_NUM);

		LOG.info("[USK] CleanUp Managed Set ................ ");

		final Tuple2<List<SetStatus>, Integer> r = cleanUpTheManagedSets(initialContext);
		Collections.sort(r.f0);
		this.currentManagedSetStatus = r.f0;
		this.activeSet = currentManagedSetStatus.get(0);
		final Integer newManagedSetNum = r.f1;

		LOG.info("[USK] CleanUp Un-Managed Set ................ ");
		cleanUpUnmanagedSets(newManagedSetNum, initialContext.getSubtaskIndex());

		this.managedSetNum.clear();
		// Only the subtask 0 would record this
		if (initialContext.getSubtaskIndex() ==  0) {
			managedSetNum.add(newManagedSetNum);
		}
...
//		}
	 ...
	 * 	1. Compute the new managed sets num
	 * 	2. Clean up the managed sets
	 */

	private Tuple2<List<SetStatus>, Integer> cleanUpTheManagedSets(USink.InitialContext initialContext) throws Exception {

		final Integer managedSetNum;
		...
		if (initialContext.isRestored()) { // The some managed sets are already assigned
			// the old managed set num
			final Integer restoreMaxParallelism = initialContext.getUnionListState(MANAGED_SETS_NUM).get().iterator().next();

			// update the managed set num
			managedSetNum = restoreMaxParallelism > initialContext.getParallelism() ? restoreMaxParallelism : initialContext.getParallelism();

			// clean up all the assigned managed sets
			for (SetStatus setStatus : managedSetStatus.get()) {
				assignedManageSets.add(setStatus);
				cleanUnmanagedElementInTheSet(setStatus, true);
			}
			if (assignedManageSets.size() == 0) {
				...
				final SetStatus setStatus = new SetStatus(setNamePrefix, initialContext.getSubtaskIndex(), 0L);
				cleanUnmanagedElementInTheSet(setStatus, true);
				assignedManageSets.add(setStatus);
			}
		} else {// The managed set are not assigned to any one so we could
			managedSetNum = initialContext.getParallelism();
			final SetStatus setStatus = new SetStatus(setNamePrefix, initialContext.getSubtaskIndex(), 0L);
			cleanUnmanagedElementInTheSet(setStatus, true);
			assignedManageSets.add(setStatus);
		}

		return Tuple2.of(assignedManageSets, managedSetNum);
	}
