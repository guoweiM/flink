 ...
 * TODO java doc.
 */
public class SinkWriterOperatorFactory<IN, SplitT> extends AbstractStreamOperatorFactory<SplitT> {

	private final USink<IN, SplitT> uSink;

	public SinkWriterOperatorFactory(USink<IN, SplitT> uSink) {
		this.uSink = uSink;
	}

	@Override
	public <T extends StreamOperator<SplitT>> T createStreamOperator(StreamOperatorParameters<SplitT> parameters) {
		final SinkWriterOperator<IN, SplitT> splitSinkOperator = new SinkWriterOperator<>(uSink);
